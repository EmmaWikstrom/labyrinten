...
const SUBJECTS = ["matte", "svenska", "engelska", "no", "so", "historia"];

const VR_FONT = "/fonts/inter/Inter-Regular.json";
const VR_FONT_IMAGE = "/fonts/inter/inter.png";
const VR_BUTTON_MATERIAL =
  "color: #145c2a; emissive: #0d3f1d; emissiveIntensity: 0.15";
...
export function setExitLocked(locked) {
  ...
  document
    .getElementById("exitText")
    .setAttribute("value", locked ? "Svara på alla frågor" : "UTGÅNG!");
  document
    .getElementById("exitText")
    .setAttribute("color", locked ? "#ff8888" : "#ffdd00");
  document
    .getElementById("exitLight")
    .setAttribute("color", locked ? "#ff4444" : "#ffdd44");
}
...
export function openModal(subject, grade, question, onAnswer, progress = null) {
  document.getElementById("modalSubject").textContent =
    `${subjectNames[subject]} | ${gradeNames[grade]}`;
...
function getCorrectFeedback(progress) {
  ...
  if (remaining === 0) return "Rätt svar! Hitta utgången.";
  if (remaining === 1) return "Rätt svar! 1 fråga kvar på nivån.";
  return `Rätt svar! ${remaining} frågor kvar på nivån.`;
}
...
function handleAnswer(btn, correct, grid, correctFeedback, callback) {
  ...
  if (correct) {
    ...
    fb.textContent = "❌ Fel svar — försök igen!";
    setTimeout(() => {
...
function openVRModal(subject, grade, question, onAnswer, correctFeedback) {
  ...
  border.setAttribute(
    "material",
    `color: ${VR_PANEL_BORDER_COLOR}; opacity: 1; side: double`,
  );
  ...
  background.setAttribute(
    "material",
    "color: #0d2a0d; opacity: 0.96; transparent: true; side: double",
  );
  ...
  subjectLabel.setAttribute("position", "0 0.5 0.02");
  ...
  questionText.setAttribute("wrap-count", "31");
  ...
  question.answers.forEach((answer, index) => {
    ...
    option.addEventListener("click", () => {
      ...
      if (correct) {
        ...
        option.setAttribute(
          "material",
          "color: #2f9e44; emissive: #1f7a34; emissiveIntensity: 0.25",
        );
        ...
        option.setAttribute(
          "material",
          "color: #b02a37; emissive: #7a1018; emissiveIntensity: 0.25",
        );
...
export function showLevelComplete(levelIndex, subject, onNext) {
  ...
  document.getElementById("lcTitle").textContent = isLast
    ? "🏆 Du klarade allt!"
    : "🎉 Nivå klar!";
  ...
  document.getElementById("nextLevelBtn").addEventListener(
    "click",
    () => {
...
function openVRLevelComplete(levelIndex, subject, onNext) {
  ...
  const title = createVRText(
    isLast ? "Du klarade allt!" : "Nivå klar!",
    "0 0.38 0.03",
    "#ffdd00",
    2.2,
    28,
  ...
  const nextButton = createVRButton(
    isLast ? "Spela igen" : "Nästa nivå",
    "0 -0.36 0.04",
    1.6,
    0.28,
    {
...
export function initStartScreen(onStart) {
  ...
  function showVRGradeChoice() {
    ...
    panel.appendChild(
      createVRText("aMAZEing minds", "0 0.88 0.03", "#90ee90", 2.4, 26),
    );
    panel.appendChild(
      createVRText("Välj årskurs", "0 0.62 0.03", "#ffcc00", 2.2, 28),
    );
  ...
  function showVRSubjectChoice() {
    ...
    panel.appendChild(
      createVRText("Välj ämne", "0 0.62 0.03", "#ffcc00", 2.2, 28),
    );
...
function createVRPanel(id, width, height, position, options = {}) {
  ...
  border.setAttribute(
    "material",
    `color: ${options.borderColor || VR_PANEL_BORDER_COLOR}; opacity: 1; side: double`,
  );
  panel.appendChild(border);
...
function createVRButton(label, position, width, height, options = {}) {
  ...
  if (options.borderColor) {
    ...
    border.setAttribute(
      "material",
      `color: ${options.borderColor}; opacity: 1; side: double`,
    );
    button.appendChild(border);
  }

  const textY = options.textY ?? VR_BUTTON_TEXT_Y;
  const text = createVRText(
    label,
    `0 ${textY} 0.02`,
